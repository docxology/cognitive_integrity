...
\begin{frame}[allowframebreaks]{Cognitive Integrity Framework: Trust
Calculus and Detection Bounds}
...
\begin{frame}[allowframebreaks]{System Model}
...
\begin{frame}[allowframebreaks]{Cognitive State}
...
\begin{frame}[allowframebreaks]{State Transition Semantics}
...
\begin{frame}[allowframebreaks]{Integrity Properties}
...
\begin{frame}[allowframebreaks]{Motivation: Why Bounded Trust Matters}
...
\begin{frame}[allowframebreaks]{Formal Trust Model}
...
\begin{frame}[allowframebreaks]{Trust Computation}
...
\begin{frame}[allowframebreaks]{Trust Algebra}
...
\begin{frame}[allowframebreaks]{Cross-Modality Trust}
...
\begin{frame}[allowframebreaks]{Federated Trust}
...
\begin{frame}[allowframebreaks]{Belief Update Semantics}
...
\begin{frame}[allowframebreaks]{Sandboxed Belief Model}
...
\begin{frame}[allowframebreaks]{Information-Theoretic Detection Bounds}
...
\begin{frame}[allowframebreaks]{CIF-AD Integration: Action-Delegation
Coupling}
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.40\textheight,keepaspectratio,alt={CIF-AD action--delegation coupling matrix showing how Functional Requirements map to Design Parameters and delegation actions, making explicit where a Goal-Hijacking transient can introduce off-diagonal coupling.}]{../figures/cif_ad_coupling.pdf}
...
\begin{frame}[allowframebreaks]{The Action-Delegation Cycle}
\protect\phantomsection\label{the-action-delegation-cycle}
...
\begin{frame}[allowframebreaks]{CIF-AD Coupling Matrix}
\protect\phantomsection\label{cif-ad-coupling-matrix}
...
\begin{frame}[allowframebreaks]{CIF-OODA Integration: Temporal Security
Across Decision Cycles}
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.40\textheight,keepaspectratio,alt={CIF-OODA phase diagram mapping the defense mechanisms onto the Observe--Orient--Decide--Act cycle, indicating at which phase each defense intervenes and its latency budget.}]{../figures/ooda_phase_diagram.pdf}
...
\begin{frame}[allowframebreaks]{OODA Loop Formalization}
\protect\phantomsection\label{ooda-loop-formalization}
...
\begin{frame}[allowframebreaks]{Phase-Specific Attack Taxonomy}
\protect\phantomsection\label{phase-specific-attack-taxonomy}
...
\begin{frame}[allowframebreaks]{OODA Temporal Coupling}
\protect\phantomsection\label{ooda-temporal-coupling}
...
The CIF defense stack (Table~23) with total latency $L_{\text{total}} \leq 141$ms satisfies the overhead constraint for OODA cycles $\tau_{\text{OODA}} \geq 1.41$s (human-scale operations). For faster OODA cycles (drone swarms: $\tau \approx 100$ms), subset selection is required:
\begin{equation}
...
\begin{frame}[allowframebreaks]{Integrative Security Guarantee}
\protect\phantomsection\label{integrative-security-guarantee}
...
\begin{frame}[allowframebreaks]{Integrative Security Guarantee}
\begin{proof}
Integrity is violated only if an attack passes all defenses in all OODA phases it targets. By defense independence (Lemma~11.7), the probability of passing all defenses is $\prod_d (1 - P_d^{\text{OODA}})$. Integrity is preserved with complementary probability.
\end{proof}
\end{frame}
