...
def create_ablation_study_figure(output_dir: Path) -> Path:
    ...
    if data_path.exists():
        ...
        for k in keys:
            label = (
                "Full CIF"
                if k == "full_cif"
                else f"− {k.replace('minus_', '').replace('_', ' ').title()}"
            )
    ...
    legend_elements = [
        ...
        Patch(facecolor="#FE6100", edgecolor="black", label="Major (−0.10 < Δ ≤ −0.07)"),
        Patch(facecolor="#FFB000", edgecolor="black", label="Moderate (−0.07 < Δ ≤ −0.04)"),
