...
def generate_attack_surface_figure(output_dir: Path) -> Path:
    ...
    # Firewall
    _draw_box(ax, (3.5, 7.5), 3, 0.9, colors["firewall"], "Cognitive Firewall\n(Filter)", fontsize=13)

    # Orchestrator
    _draw_box(ax, (3.5, 5.3), 3, 1.4, colors["orchestrator"], "Orchestrator\n(Coordinator)", fontsize=13)
    ax.text(6.6, 6.0, r"$\Omega_5$", ha="center", va="center", fontsize=16, color="black", weight='bold')
    ...
    ax.text(9.2, 6.9, "• Web APIs", ha="center", va="center", fontsize=11, color="white", weight='bold')
    ax.text(9.2, 6.5, "• Tools", ha="center", va="center", fontsize=11, color="white", weight='bold')
    ax.text(9.2, 6.1, "• RAG DBs", ha="center", va="center", fontsize=11, color="white", weight='bold')
    ax.text(9.2, 8.9, r"$\Omega_2$", ha="center", va="center", fontsize=16, color="black", weight='bold')

    # Shared state (bottom)
    _draw_box(ax, (2, 0.8), 6, 1.0, colors["shared"], r"Shared Belief State / Memory ($\Omega_4$)", fontsize=14)
    ...
    ax.annotate("", xy=(3.5, 3.6), xytext=(3, 3.6), arrowprops=dict(arrowstyle="<->", color="#9B59B6", lw=2, linestyle='--'))
    ax.annotate("", xy=(5.5, 3.6), xytext=(5, 3.6), arrowprops=dict(arrowstyle="<->", color="#9B59B6", lw=2, linestyle='--'))
    ax.annotate("", xy=(7.5, 3.6), xytext=(7, 3.6), arrowprops=dict(arrowstyle="<->", color="#9B59B6", lw=2, linestyle='--'))
    
    # Add coordination flow label
    ax.text(5, 3.8, r"Inter-agent Coordination ($\Omega_4$)", fontsize=11, ha="center", 
            color="#666", style="italic", backgroundcolor="#f0f0f0")
    ...
    for i, (color, label) in enumerate(legend_items):
        ax.add_patch(
            plt.Rectangle(
                (0.2, 9.5 - i * 0.6), 0.4, 0.4, facecolor=color, edgecolor="black"
            )
        )
