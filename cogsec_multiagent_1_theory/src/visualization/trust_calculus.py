...
def create_trust_calculus_figure(output_dir: Path) -> Path:
    ...
    formula_box = FancyBboxPatch((0.5, 7), 9, 2.5, boxstyle="round,pad=0.05",
                                  facecolor="#F8F9FA", edgecolor=colors["header"], linewidth=2)
    ax2.add_patch(formula_box)
    ax2.text(5, 8.8, "T'(a→b) = α·T(a→b) + β·outcome + γ·consensus",
             ha="center", va="center", fontsize=13, fontfamily="monospace", fontweight="bold", color=colors["header"])
    ax2.text(5, 8.0, "where: α + β + γ = 1 (normalization)", ha="center", va="center", fontsize=12, style="italic", color="#5D6D7E")
    ...
    for name, formula, color, x, y in components:
        box = FancyBboxPatch((x - 1.2, y - 0.8), 2.4, 2, boxstyle="round,pad=0.02",
                              facecolor=color, edgecolor="black", linewidth=1.5, alpha=0.8)
        ax2.add_patch(box)
        ax2.text(x, y + 0.4, name, ha="center", va="center", fontsize=12, fontweight="bold", color="white")
        ax2.text(x, y - 0.3, formula, ha="center", va="center", fontsize=11, fontfamily="monospace", color="white")

    result_box = FancyBboxPatch((3, 0.8), 4, 1.2, boxstyle="round,pad=0.02",
                                 facecolor=colors["trust_high"], edgecolor="black", linewidth=2)
    ax2.add_patch(result_box)
    ax2.text(5, 1.4, "Updated Trust T'(a→b)", ha="center", va="center", fontsize=13, fontweight="bold", color="white")
    ...
    ax3.set_ylim(0, 8)
    ax3.axis("off")
    ax3.set_title("C. Bounded Delegation Chain (Theorem 3.1)", fontsize=14, fontweight="bold", pad=12)
    ...
    for i, (agent, trust, (x, y)) in enumerate(zip(agents, trust_values, positions)):
        color = colors["trust_high"] if trust >= 0.8 else colors["trust_med"] if trust >= 0.6 else colors["trust_low"]
        circle = Circle((x, y), 0.8, facecolor=color, edgecolor="black", linewidth=2)
        ax3.add_patch(circle)
        ax3.text(x, y + 0.1, agent, ha="center", va="center", fontsize=16, fontweight="bold", color="white")
        ax3.text(x, y - 0.35, f"T={trust:.2f}", ha="center", va="center", fontsize=11, color="white")
        if i < len(agents) - 1:
            ax3.annotate("", xy=(positions[i + 1][0] - 0.9, y), xytext=(x + 0.9, y),
                        arrowprops=dict(arrowstyle="-|>", color=colors["header"], lw=2.5, mutation_scale=18))
            ax3.text((x + positions[i + 1][0]) / 2, y + 0.6, "δ=0.9",
                    ha="center", va="center", fontsize=10, color="#7F8C8D", style="italic")

    ax3.text(7, 3, "T(A→E) ≤ δ⁴·T(A→B) = 0.9⁴ × 1.0 = 0.66", ha="center", va="center",
             fontsize=13, fontfamily="monospace", color=colors["header"])
    ax3.text(7, 2, "Trust bounded exponentially: prevents trust amplification",
    ...
    ax4.text(7, 7, "Attack Attempt: Malicious → Trusted → Target",
             ha="center", va="center", fontsize=13, fontweight="bold", color=colors["attack"])
    ...
    target_circle = Circle((12, 4.5), 0.9, facecolor=colors["trust_high"], edgecolor="black", linewidth=2)
    ...
    ax4.annotate("", xy=(6, 4.5), xytext=(3, 4.5),
    ...
    ax4.annotate("", xy=(11, 4.5), xytext=(8, 4.5),
    ...
    result_box = FancyBboxPatch((2, 0.5), 10, 1.5, boxstyle="round,pad=0.02",
                                 facecolor=colors["trust_high"], edgecolor="black", linewidth=2, alpha=0.9)
    ax4.add_patch(result_box)
    ax4.text(7, 1.5, "T(M→V) ≤ δ · T(M→T) = 0.9 × 0.3 = 0.27 < τ",
             ha="center", va="center", fontsize=13, fontfamily="monospace", fontweight="bold", color="white")
    ax4.text(7, 0.9, "Delegated trust BLOCKED - below threshold",
