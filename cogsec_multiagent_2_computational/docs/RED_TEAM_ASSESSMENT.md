...
| H3 | **`statistical_results.json` mislabeled `real_pipeline`; stale vs head; missing provenance; absurd H1/cohen's-d vs invented control.** | ✅ **RESOLVED (provenance/labeling).** Regenerated `statistical_results.json` from HEAD. The `data_origin`/`source_script`/`generated_by` keys are retained as **preservation markers** (they tell the anti-clobber guard this is authoritative evidence), but the *false* meaning was corrected: `data_origin` no longer implies a measurement, and an honest nested `provenance` block (baseline simulated, cohens_d vs simulated control) is present. The effect sizes themselves (d≈62) remain **OPEN** — they are artifacts of comparing near-1.0 simulation rates to an invented `N(0.03,0.02)` control and should not be reported as real evidence; the manuscript does not cite them. |
