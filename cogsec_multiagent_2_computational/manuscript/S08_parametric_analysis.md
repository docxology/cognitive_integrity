...
\item **Gap to empirical**: The prototype pipeline achieves $\sim$12\% TPR and multi-seed stability analysis shows $\sim$44\% mean detection rate on the current corpus, yielding a 51--88 percentage-point gap to parametric predictions. This gap reflects the distinction between CIF's formal coverage guarantees and the current adapter implementations' maturity.
\end{enumerate}
