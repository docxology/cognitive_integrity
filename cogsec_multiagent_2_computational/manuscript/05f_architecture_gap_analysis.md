...
\item \emph{Consensus}, Level 1--2$\to$3: deploy with real agent voting rather than mock votes; calibrate against the coordination-attack subcorpus. Projected gain: $+12$ pp on coordination attacks. Rationale: the formal BFT guarantees from Part 1 are strong, but the adapter currently bypasses them. Wiring real votes into the adapter is an engineering task, not a research task.
\item \emph{Sandbox}, Level 2$\to$3: calibrate corroboration threshold $\kappa$ per architecture based on observed false-positive rate on the current corpus. Projected gain: $+8$ pp on belief-manipulation and trust-exploitation attacks. Rationale: $\kappa$ is currently uniform across architectures, but the architecture adapters differ in their corroboration-signal rate.
