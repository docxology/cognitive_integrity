...
This projection was written against a real-pipeline rate of $44.8\%$, and the arithmetic below is retained as it stood because the module-maturity argument does not depend on the starting point. The starting point has moved: the multi-seed arm now measures $86.3\%$, which is already inside the $80$--$86\%$ band the projected $+35$ to $+41$ percentage points were supposed to reach. The projection is therefore superseded by measurement rather than confirmed by it, and what remains to be closed is the gap to the $96$--$100\%$ design-level ceiling, not the gap this section was written to explain. Note the target levels differ by module: Detection reaches Level 4, while Consensus, Sandbox and Provenance reach Level 3. Realising the full parametric ceiling would require Level 4--5 across the board, which this roadmap does not attempt. This projection does not require any change to the CIF formal framework; it quantifies the engineering maturity required to realize the existing parametric ceiling. The Part 3 deployment guide provides the operational steps for each of these upgrades in order.
