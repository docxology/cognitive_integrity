...
We evaluated pipeline detection rate stability across 30 random seeds on the Claude Code architecture using the full defense pipeline against the 950-attack corpus.

**Table: Multi-seed pipeline detection rate summary (Claude Code, $N=30$ seeds).** {#tab:multi-seed-summary}

| Metric  | Value |
| --- | --- |
| Mean Detection Rate | 0.447 |
...
We evaluated 5 attacks across 2 architecture topologies (Claude Code hub-spoke, CrewAI chain) with full multiagent propagation:
...
Colony benchmarks evaluate CIF at scale across five adversarial scenarios with varying agent counts and adversary populations:
