...
$$\mathrm{DR}(\theta^*, a) \geq \mathrm{DR}(\theta^*, a^*) \geq \mathrm{DR}(\theta, a^*)$$ {#eq:nash-equilibrium-condition}
...
$$\max_{\theta} \min_a \mathrm{DR}(\theta, a) = \min_a \max_\theta \mathrm{DR}(\theta, a)$$ {#eq:minimax-theorem-at}
...
$$\|\theta^{(k)} - \theta^*\|_2 \leq (1 - \alpha L)^k \|\theta^{(0)} - \theta^*\|_2$$ {#eq:at-convergence-rate}
\end{theorem}
{#thm:at-convergence}
...
$$\theta^{(k+1)} = \theta^{(k)} + \alpha G_F(\theta^{(k)})^{-1} \nabla_\theta \mathrm{DR}$$ {#eq:natural-gradient-ascent}
...
$$\|\theta^{(k+1)}_{\text{NG}} - \theta^*\|_G \leq C \|\theta^{(k)}_{\text{NG}} - \theta^*\|_G^2$$ {#eq:natural-gradient-quadratic-convergence}
...
$$\mathrm{DR}(\theta^{(K)}, a) \geq \mathrm{DR}(\theta^*, a) -
  \epsilon_j \cdot (1 - \alpha L)^K \|\theta^{(0)} - \theta^*\|_2$$ {#eq:at-robustness-bound}
...
$$r_{\text{eff}}^{(K)} \geq r_{\text{eff}}^{(0)} + \sum_{k=1}^K \Delta r^{(k)}$$ {#eq:at-radius-monotone}
