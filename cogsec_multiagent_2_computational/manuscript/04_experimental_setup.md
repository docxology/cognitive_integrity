...
![Detection performance. **Panel A** reports the ablation run (\texttt{output/data/ablation\_results.json}): each mechanism's true-positive rate in isolation, and the full pipeline's. Only the four mechanisms whose solo rates the ablation records are shown. Measured false-positive rate is exactly zero for every configuration in this run, so the FPR series is present but sits on the axis; F1 is computed from the measured pair, which at zero FPR reduces to $2r/(1+r)$. The series composition rule $1 - \prod(1 - r_i)$ predicts 12.6\% from the four solo rates against a measured 12.2\% --- close agreement, and the sharper test of the composition algebra than any single number. **Panel B** reports a different arm: per-architecture detection rate by attack category from the evaluation pipeline (\texttt{output/data/detection\_data.json}), with the bootstrap 95\% intervals that artifact ships. The two panels are not comparable and are not meant to be --- the ablation runs a deliberately hostile stratified subsample, the evaluation runs the full corpus --- which is why the ordinate differs by an order of magnitude between them.](figures/detection_performance.pdf){#fig:detection-performance width=95%}
