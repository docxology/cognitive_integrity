...
Our pipeline and ablation evaluations use 3--10 agent configurations, and the colony benchmarks reach 20--100 agents (\cref{tab:colony-benchmarks}). Beyond that range, three bottlenecks constrain scaling:
...
The ablation data (\cref{tab:component-removal}) reveals a clear incremental cost-benefit curve for the current adapter implementations. The Invariants module alone accounts for essentially all of it, and the curve is therefore not incremental at all: one component carries the pipeline and the rest are, on this corpus, redundant to it.000$ (about 42\% of full-pipeline TPR on the ablation corpus); Trust Calculus ($\approx -0.020$) is the next-largest removal effect, and Firewall, Invariants and Tripwire tie behind it. Detection and Trust Calculus together account for about 82\% of the summed negative $\Delta\text{TPR}$ magnitude in this run. Two pairs tie for the strongest synergy---Firewall + Detection and Tripwire + Detection, both $\approx +0.031$ (\cref{tab:real-synergy}), combining canary-belief shift detection with text-feature analysis.

The gap between the full pipeline TPR ($\sim$12\% on the 98-attack ablation corpus, $\sim$45\% mean across 30 seeds) and the parametric ceiling (96--100\%; \cref{sec:parametric-analysis}) quantifies the adapter implementation gap. The lower-ranked components currently show modest marginal contribution (three tied at $\Delta\text{TPR} \approx -0.010$, three at exactly $0.000$) on the evaluation corpus, suggesting that the attack distribution does not sufficiently exercise these mechanisms---or that their current adapter implementations require further tuning. The practical deployment implications of these findings are explored in Part 3.
...
The static Nash analysis, however, assumes a fixed attack distribution and fixed defender capability. Running the arms-race simulation (\texttt{arms\_race\_simulation()} in \texttt{src/analysis/game\_theory.py}) under adversarial adaptation reveals that without defender retraining the effective detection rate degrades at approximately 2\% per attacker adaptation cycle. Periodic defender retraining every five cycles with 3 pp recovery per event stabilizes the long-run equilibrium at $\sim 0.52$---a 4 pp degradation from the static Nash value but a stable operating regime. Without any defender maintenance, the arms race asymptotes toward zero detection over approximately 30 cycles, consistent with the degradation bounds derived in Part 1's Distribution Shift Degradation property.
