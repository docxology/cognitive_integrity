...
\makeatother
\vfill
\includegraphics[width=0.98\textwidth,height=0.35\textheight,keepaspectratio,alt={Bar chart of goal-hijacking attack patterns across ten critical domains, color-coded by pattern: FR polarity inversion (5 domains), context boundary violation (3), and constraint relaxation (2).}]{\detokenize{../figures/domain_coverage.png}}
\vfill
\end{titlepage}
\thispagestyle{empty}
\tableofcontents
...
axis rather than expose it, and a real label needs a per-sample
judgement this corpus has never had. The prediction is worth testing and
is tracked as such; it has not been tested.
...
installed anywhere. An earlier revision of this table reported 3+
identity, 5+ boundary, 2+ principal and 1 temporal canary per agent;
...
from the ceiling to the 95.9\% the pipeline reaches on the 100-attack
...
Violation (4/10). Each bar marks the single dominant pattern for that
domain; the right-margin callouts give per-pattern totals, which match
the domain-by-domain table below.}\label{fig:domain-coverage}
\end{figure}

\subsection{10.1 Cross-Domain Attack Pattern
...
\subsection{10.2 The Independence Axiom Under Adversarial
...
\subsection{10.3 OODA Transient Dynamics}\label{ooda-transient-dynamics}
...
\subsection{10.4 CIF Mechanism Coverage
...
\subsection{10.5 Novel Defense Patterns}\label{novel-defense-patterns}
...
\subsection{10.6 Byzantine Fault Tolerance
...
\subsection{10.7 Comparison with Existing
...
\subsection{10.8 Empirical Grounding: Real-World
...
\subsection{10.9 Limitations}\label{sec:limitations_discussion}
